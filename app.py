...
def getRepPaths(url, path=''):
    paths = []
    try:
        browsingPage = requests.get(url + path).text
    except:
...
peepTwitter = 'http://twitter.com/example'
version = '0.3'
revision = '275'
stats = ''
pdf = None
...
try:
    ...
    if options.version:
        ...
        try:
            remotePeepContent = requests.get(rawRepURL + 'peepdf.py').text
        except:
        ...
        if localVersion == newVersion:
            ...
            for path in pathNames:
                try:
                    fileContent = requests.get(rawRepURL + path).text
                except:
    ...
    if os.path.exists(errorsFile):
        ...
        message += '\t- Sending the file "%s" to the author %s' % (
            errorsFile, newLine)
        message += '\t- And/Or creating an issue on the project webpage' + newLine
